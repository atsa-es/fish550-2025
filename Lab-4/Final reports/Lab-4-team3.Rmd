...
We chose to focus on the eight biological indicators included in the "stoplight" dataset to assess whether there are underlying regimes of survival and mortality for biota being sampled. We begin by reshaping data to suit HMM requirements before proceeding to fit 2-state and 3-state HMMs and compare.
...
Here we fit a two state HMM to the set of eight biological indicators
```{r 2state, echo=FALSE}
...
Here we fit a three state HMM to the set of eight biological indicators
```{r 3state, echo=FALSE}
...
Transition probabilities for the PDO model are fairly evenly distributed, with perhaps a slight tendency towards the first state. 
```{r, echo=FALSE}
trans_mat
```

Transition probabilities for the 2-state biological indicator model are skewed heavily in favor of the system existing in the second state. 
```{r, echo=FALSE}
hmm2$par()$tpm
```

Transition probabilities for the 3-state biological indicator model are perplexing. The model identifies the same probabilites of movement between state 1 and state 2 as the two state model. This is made possible because the model leaves virtually no room for movement from either of these states into state 3. If the system were to find itself in state 3 this model estimated transitioning away from it would be difficult but not impossible, and once this occured the system would be very unlikely to return to that state.
```{r, echo=FALSE}
hmm3$par()$tpm
```
...
In the case of the PDO model, we seem to be in the first state more than the second. This is consistent with the estimates of long term probabilities where we spend 2/3s of the time in one state and 1/3 in the other. 
...
The results from the two state biological indicators model is consistent with the transition matrix indicating a strong tendency towards state 2.
```{r, echo=FALSE}
ggplot(data, aes(x=year, y=est_state2, col = est_state2)) + 
  geom_point() + 
  ylab("") + 
  xlab("") +
  scale_x_discrete(breaks=c("1998", "2003", "2008", "2013", "2018")) +
  theme_classic()
```

Results from the three state biological indicators model mirror those from the two state model. This is consistent with the transition matrix leaving very little chance for the model to move into a third state.
```{r, echo=FALSE}
...
Modeling multistate biological data with HMMs revealed interesting results. A two state modeling approach heavily favored the second state but allowed movement between the two. The addition of a third state however made no difference in the model's estimation of regime, with the third state being basically ignored by the model. This is perhaps evidence that, based on the biological indicators sampled, the system moves between one of two states (perhaps healthy and unhealthy).
